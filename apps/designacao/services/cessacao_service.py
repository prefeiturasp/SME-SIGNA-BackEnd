...
class CessacaoService:
    ...
    def criar(data: dict) -> AtoAdministrativo:
        ...
        if ato_pai.filhos.filter(tipo=AtoAdministrativo.Tipo.CESSACAO, ativo=True).exists():
            raise ValidationError({'ato_pai': 'Esta designação já possui uma cessação ativa.'})

        data_ato = {k: v for k, v in data.items() if k in _CAMPOS_ATO}
        data_detalhe = {k: v for k, v in data.items() if k not in _CAMPOS_ATO and k != 'ato_pai'}
