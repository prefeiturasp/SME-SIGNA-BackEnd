...
def auth_client(db):
    ...
    user = User.objects.create_user(username="test_insub", password=password)
    client = APIClient()
...
def test_create_insubsistencia_de_designacao(auth_client):
    """Verifica create insubsistencia de designacao."""
...
def test_create_insubsistencia_registra_criado_por(auth_client):
    ...
    d = criar_ato_designacao()
    user = User.objects.get(username="test_insub")
...
@pytest.mark.django_db
def test_create_insubsistencia_de_cessacao(auth_client):
    """Verifica create insubsistencia de cessacao."""
    d = criar_ato_designacao()
...
@pytest.mark.django_db
def test_create_insubsistencia_ato_pai_invalido(auth_client):
    """Verifica create insubsistencia ato pai invalido."""
    url = reverse("designacao:insubsistencias")
...
@pytest.mark.django_db
def test_create_insubsistencia_duplicada_rejeita(auth_client):
    """Verifica create insubsistencia duplicada rejeita."""
    d = criar_ato_designacao()
...
@pytest.mark.django_db
def test_create_insubsistencia_marca_pai_como_inativo(auth_client):
    """Verifica create insubsistencia marca pai como inativo."""
    d = criar_ato_designacao()
    url = reverse("designacao:insubsistencias")
...
@pytest.mark.django_db
def test_list_insubsistencias(auth_client):
...
@pytest.mark.django_db
def test_retrieve_insubsistencia(auth_client):
...
@pytest.mark.django_db
def test_retrieve_insubsistencia_nao_encontrada(auth_client):
    """Verifica retrieve insubsistencia nao encontrada."""
    url = reverse("designacao:insubsistencia-detail", args=[9999])
    response = auth_client.get(url)
    assert response.status_code == 404


@pytest.mark.django_db
def test_destroy_insubsistencia_restaura_pai(auth_client):
    """Verifica destroy insubsistencia restaura pai."""
    d = criar_ato_designacao()
    insub = criar_ato_insubsistencia(d)
    d.ativo = False
    d.save(update_fields=["ativo"])
...
@pytest.mark.django_db
def test_create_insubsistencia_de_apostila_cria_detalhe(auth_client):
...
@pytest.mark.django_db
def test_create_insubsistencia_de_apostila_retorna_texto(auth_client):
...
@pytest.mark.django_db
def test_insubsistencia_de_designacao_texto_apostila_nulo(auth_client):
...
@pytest.mark.django_db
def test_retrieve_insubsistencia_apostila_exibe_texto(auth_client):
